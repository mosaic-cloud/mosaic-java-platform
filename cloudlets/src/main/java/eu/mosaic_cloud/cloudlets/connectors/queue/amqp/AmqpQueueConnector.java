 ...
 */
public abstract class AmqpQueueConnector<Context, Message> implements IAmqpQueueConnector<Context> {
     ...
     */
    public AmqpQueueConnector(IConfiguration config, ICloudletController<Context> cloudlet,
            Class<Message> dataClass, boolean consumer, DataEncoder<Message> encoder) {
        synchronized (this.monitor) {
            ...
            this.exclusive = ConfigUtils.resolveParameter(accessorConfig,
                    ConfigProperties.getString("AmqpQueueConnector.6"), Boolean.class, true); //$NON-NLS-1$ 
            this.autoDelete = ConfigUtils.resolveParameter(accessorConfig,
                    ConfigProperties.getString("AmqpQueueConnector.7"), Boolean.class, true); //$NON-NLS-1$
            this.passive = ConfigUtils.resolveParameter(accessorConfig,
                    ConfigProperties.getString("AmqpQueueConnector.8"), Boolean.class, false); //$NON-NLS-1$ 
            this.durable = ConfigUtils.resolveParameter(accessorConfig,
                    ConfigProperties.getString("AmqpQueueConnector.9"), Boolean.class, false); //$NON-NLS-1$ 
            this.logger.trace("Queue accessor for exchange '" + this.exchange + "', routing key '"
                    + this.routingKey + "' and queue '" + this.queue + "'");
        }
    }

    public CallbackCompletion<Void> initialize(IConnectorCallback<Context> callback,
            Context context, ThreadingContext threading) {
        synchronized (this.monitor) {
            IConnectorCallback<Context> proxy = this.cloudlet.buildCallbackInvoker(callback,
            ...
            try {
                ...
                if (!CloudletComponentResourceFinder.getResourceFinder().findResource(
                    ...
                    throw new ConnectorException("Cannot find a resource of type "
                            + ResourceType.AMQP.toString());
                }
                this.connector = eu.mosaic_cloud.connectors.queue.amqp.AmqpQueueConnector.create(
                        this.configuration, threading);
                ...
                proxy.initializeSucceeded(AmqpQueueConnector.this.cloudletContext, arguments);
    ...
    public CallbackCompletion<Void> destroy() {
        synchronized (this.monitor) {
            ...
            IConnectorCallback<Context> proxy = this.cloudlet.buildCallbackInvoker(callback,
                    IConnectorCallback.class);
            try {
                this.logger.trace("AmqpQueueConnector is destroying the connector...");
                this.connector.destroy();
                CallbackArguments<Context> arguments = new GenericCallbackCompletionArguments<Context, Boolean>(
                        AmqpQueueConnector.this.cloudlet, true);
                proxy.destroySucceeded(this.cloudletContext, arguments);
                this.logger.trace("AmqpQueueConnector destroyed successfully.");
            } catch (Throwable e) {
                ExceptionTracer.traceDeferred(e);
                CallbackArguments<Context> arguments = new GenericCallbackCompletionArguments<Context, Boolean>(
                        AmqpQueueConnector.this.cloudlet, e);
    ...
    protected void declareExchange(final IAmqpQueueConnectorCallback<Context> callback) {
        IOperationCompletionHandler<Boolean> cHandler = new IOperationCompletionHandler<Boolean>() {
            ...
            public void onFailure(Throwable error) {
                ...
                callback.registerFailed(AmqpQueueConnector.this.cloudletContext, arguments);
            }
        };
        ...
        getConnector().declareExchange(this.exchange, this.exchangeType, this.durable,
    ...
    private void declareQueue(final IAmqpQueueConnectorCallback<Context> callback) {
        ...
        getConnector().declareQueue(this.queue, this.exclusive, this.durable, this.autoDelete,
                this.passive, cHandlers, this.cloudlet.getResponseInvocationHandler(cHandler));
    ...
    private void bindQueue(final IAmqpQueueConnectorCallback<Context> callback) {
        ...
        getConnector().bindQueue(this.exchange, this.queue, this.routingKey, cHandlers,
                this.cloudlet.getResponseInvocationHandler(cHandler));
    }
    ...
    protected abstract void finishRegister(IAmqpQueueConnectorCallback<Context> callback);
     ...
     * 
     */
    final class ConnectionOpenHandler implements IOperationCompletionHandler<Boolean> {
        ...
        public void onSuccess(Boolean result) {
            ...
            synchronized (AmqpQueueConnector.this.monitor) {
                this.callback.initializeSucceeded(AmqpQueueConnector.this.cloudletContext,
                        arguments);
        ...
        public void onFailure(Throwable error) {
            CallbackArguments<Context> arguments = new GenericCallbackCompletionArguments<Context, Boolean>(
                    AmqpQueueConnector.this.cloudlet, error);
            this.callback.initializeFailed(AmqpQueueConnector.this.cloudletContext, arguments);
        }
     ...
     * 
     */
    final class ConnectionCloseHandler implements IOperationCompletionHandler<Boolean> {
        ...
        public void onSuccess(Boolean result) {
            CallbackArguments<Context> arguments = new GenericCallbackCompletionArguments<Context, Boolean>(
                    AmqpQueueConnector.this.cloudlet, result);
            synchronized (AmqpQueueConnector.this.monitor) {
                this.callback.destroySucceeded(AmqpQueueConnector.this.cloudletContext, arguments);
        ...
        public void onFailure(Throwable error) {
            CallbackArguments<Context> arguments = new GenericCallbackCompletionArguments<Context, Boolean>(
                    AmqpQueueConnector.this.cloudlet, error);
            this.callback.destroyFailed(AmqpQueueConnector.this.cloudletContext, arguments);
        }
