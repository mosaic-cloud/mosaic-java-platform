...
public class CloudletManager
{
	 ...
	 */
	public CloudletManager (final CallbackReactor reactor, final ThreadingContext threading, final ExceptionTracer exceptions, final ClassLoader classLoader, final IConfiguration configuration)
	{
		super ();
	 ...
	 */
	private void createCloudletInstance ()
			throws CloudletException
	{
		Class<?> handlerClasz;
		Class<?> stateClasz;
		...
		try {
			final String cloudletClass = ConfigUtils.resolveParameter (this.configuration, ConfigProperties.getString ("CloudletComponentCallbacks.8"), String.class, ""); //$NON-NLS-1$ //$NON-NLS-2$
			if (cloudletClass.equals ("")) {
				throw new CloudletException ("The configuration file " //$NON-NLS-1$
						+ this.configuration.toString () + " does not specify a handler class for cloudlet " //$NON-NLS-1$
						+ cloudletClass + "."); //$NON-NLS-1$
			}
			final String cloudletStateClass = ConfigUtils.resolveParameter (this.configuration, ConfigProperties.getString ("CloudletComponentCallbacks.8"), String.class, ""); //$NON-NLS-1$ //$NON-NLS-2$
			if (cloudletStateClass.equals ("")) {
				throw new CloudletException ("The configuration file " //$NON-NLS-1$
						+ this.configuration.toString () + " does not specify a context class for cloudlet " //$NON-NLS-1$
						+ cloudletClass + "."); //$NON-NLS-1$
			}
			final String resourceFile = ConfigUtils.resolveParameter (this.configuration, ConfigProperties.getString ("CloudletComponentCallbacks.10"), String.class, ""); //$NON-NLS-1$ //$NON-NLS-2$
			...
			final CloudletEnvironment environment = CloudletEnvironment.create (resourceConfig, handlerClasz, stateClasz, this.classLoader, DefaultConnectorsFactory.create (null, connectorEnvironment), this.reactor, this.threading, this.exceptions);
