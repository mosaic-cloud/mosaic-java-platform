 ...
 * 
 */
public final class CloudletComponentCallbacks
		implements
			ComponentCallbacks,
			CallbackHandler
{
	 ...
	 * with the connectors.
	 */
	public CloudletComponentCallbacks (final ComponentEnvironment componentEnvironment)
	{
		super ();
		...
		final IConfiguration configuration = PropertyTypeConfiguration.create (CloudletComponentCallbacks.class.getClassLoader (), "eu/mosaic_cloud/cloudlets/cloudlet-component.properties"); //$NON-NLS-1$
		this.selfGroup = ComponentIdentifier.resolve (ConfigUtils.resolveParameter (configuration, ConfigProperties.getString ("CloudletComponent.3"), String.class, "")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public CallbackCompletion<Void> callReturned (final ComponentController component, final ComponentCallReply reply)
	{
		...
		if (this.pendingReferences.containsKey (reply.reference)) {
			final Trigger<ComponentCallReply> trigger = this.pendingReferences.remove (reply.reference);
	...
	{
		CloudletComponentCallbacks.logger.trace ("ComponentController container failed " + exception.getMessage ());
		Preconditions.checkState (this.component == component);
	 ...
	 */
	public ChannelData findDriver (final ResourceType type)
	{
		...
		this.component.call (componentId, ComponentCallRequest.create (ConfigProperties.getString ("CloudletComponent.7"), null, callReference)); //$NON-NLS-1$
		try {
	...
	public CallbackCompletion<Void> registerReturned (final ComponentController component, final ComponentCallReference reference, final boolean ok)
	{
		...
		if (pendingReply != null) {
			...
			CloudletComponentCallbacks.logger.info ("Container component callback registered to group " + this.selfGroup); //$NON-NLS-1$
			...
			if (properties != null) {
				...
				final List<CloudletManager> containers = this.startCloudlet (loader, properties);
				if (containers != null) {
	...
	private List<CloudletManager> startCloudlet (final ClassLoader loader, final String configurationFile)
	{
		final IConfiguration configuration = PropertyTypeConfiguration.create (loader, configurationFile);
		if (configuration == null) {
			CloudletComponentCallbacks.logger.error ("Cloudlet configuration file " + configurationFile + " is missing.");
			return null;
		}
		final int noInstances = ConfigUtils.resolveParameter (configuration, ConfigProperties.getString ("CloudletComponent.11"), Integer.class, 1);
		final List<CloudletManager> containers = new ArrayList<CloudletManager> ();
		for (int i = 0; i < noInstances; i++) {
			final CloudletManager container = new CloudletManager (this.componentEnvironment.reactor, this.componentEnvironment.threading, this.componentEnvironment.exceptions, loader, configuration);
			try {
				container.start ();
				containers.add (container);
				CloudletComponentCallbacks.logger.trace ("Starting cloudlet with config file " + configurationFile);
	...
	private static MosaicLogger logger = MosaicLogger.createLogger (CloudletComponentCallbacks.class);
	...
	public static enum ResourceType
	{
		...
		KEY_VALUE ("kvstore"),
		MEMCACHED ("kvstore");
		ResourceType (final String configPrefix)
		{
