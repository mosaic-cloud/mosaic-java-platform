...
if test -h "${_self}" ; then
	...
	if test "${_self_next_basename}" == 'do.bash' ; then
		_self_do="${_self_next}"
		_self_do_realpath="${_self_next_realpath}"
		_self_do_realpath_dirname="${_self_next_realpath_dirname}"
	else
		echo "[ee] arg0 \`${_self}\` is expected to be a symlink to the \`do.bash\` script; aborting!" >&2
		exit 1
	fi
	if test "${_self_dirname_realpath_basename}" == scripts ; then
		...
		echo "[ee] arg0 \`${_self}\` is expected to be a symlink to the \`do.bash\` script inside the \`scripts\` folder inside the project's workbench; aborting!" >&2
		exit 1
	fi
	...
	if test -f "${_self_workbench}/scripts/env.bash" ; then
		_self_env="${_self_workbench}/scripts/env.bash"
	elif test -f "${_self_do_realpath_dirname}/env.bash" ; then
		_self_env="${_self_do_realpath_dirname}/env.bash"
	else
		echo "[ww] cannot locate \`env.bash\` script in either \`${_self_workbench}/scripts\` or \`${_self_do_dirname_realpath}\` folders; ignoring!" >&2
	fi
