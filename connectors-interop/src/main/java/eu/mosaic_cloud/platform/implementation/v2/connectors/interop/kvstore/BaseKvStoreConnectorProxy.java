...
package eu.mosaic_cloud.platform.implementation.v2.connectors.interop.kvstore;
...
import eu.mosaic_cloud.platform.implementation.v2.connectors.interop.core.BaseConnectorProxy;
...
public abstract class BaseKvStoreConnectorProxy<TValue extends Object>
			...
			implements
				eu.mosaic_cloud.platform.implementation.v2.connectors.kvstore.BaseKvStoreConnectorProxy<TValue>
{
	...
	public CallbackCompletion<Void> delete (final String key, final CallbackCompletionTrigger<Void> trigger) {
		...
		return (this.sendRequest (message, token, trigger));
	}
	...
	public CallbackCompletion<Void> destroy (final CallbackCompletionTrigger<Void> trigger) {
		...
		requestBuilder.setToken (token);
		return (this.disconnect (new Message (KeyValueMessage.ABORTED, requestBuilder.build ()), trigger));
	}
	
	@Override
	@SuppressWarnings ("unchecked")
	public CallbackCompletion<TValue> get (final String key, final CallbackCompletionTrigger<TValue> trigger) {
		return (this.sendGetRequest (Arrays.asList (key), trigger));
	}
	
	public CallbackCompletion<Void> set (final String key, final int exp, final TValue data, final CallbackCompletionTrigger<Void> trigger) {
		return (this.sendSetRequest (key, data, exp, trigger));
	}
	
	@Override
	public CallbackCompletion<Void> set (final String key, final TValue data, final CallbackCompletionTrigger<Void> trigger) {
		return (this.set (key, 0, data, trigger));
	}
	...
	protected <TOutcome> CallbackCompletion<TOutcome> sendGetRequest (final List<String> keys, final CallbackCompletionTrigger<TOutcome> trigger) {
		final CompletionToken token = this.generateToken ();
	...
	protected CallbackCompletion<Void> sendSetRequest (final String key, final TValue data, final int exp, final CallbackCompletionTrigger<Void> trigger) {
		final CompletionToken token = this.generateToken ();
