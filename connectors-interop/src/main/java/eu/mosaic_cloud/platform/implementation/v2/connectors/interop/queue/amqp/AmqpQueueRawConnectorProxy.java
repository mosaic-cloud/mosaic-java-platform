...
import eu.mosaic_cloud.platform.v2.connectors.queue.amqp.AmqpQueueRawConnector;
				...
				AmqpQueueRawConnector
{
	...
	public CallbackCompletion<Void> ack (final long delivery, final boolean multiple) {
		...
		return (this.sendRequest (message, token, null));
	}
	...
	public CallbackCompletion<Void> cancel (final String consumer) {
		...
		final CallbackCompletionTrigger<Void> trigger = CallbackCompletionTrigger.create ();
		this.pendingRequests.register (consumer + "//cancel", trigger);
		this.sendRequest (message, token, null);
	...
	public CallbackCompletion<Void> consume (final String queue, final String consumer, final boolean exclusive, final boolean autoAck, final AmqpQueueRawConsumerCallback consumerCallback) {
		...
		final CallbackCompletionTrigger<Void> trigger = CallbackCompletionTrigger.create ();
		this.pendingRequests.register (consumer + "//consume", trigger);
		this.sendRequest (message, token, null);
		return (trigger.completion);
	}
	...
	public CallbackCompletion<Void> destroy () {
		return (this.destroy (CallbackCompletionTrigger.<Void> create ()));
	}
	...
	public CallbackCompletion<Void> destroy (final CallbackCompletionTrigger<Void> trigger) {
		...
		return (this.disconnect (null, trigger));
	}
	...
	public CallbackCompletion<Void> initialize () {
		return (this.initialize (CallbackCompletionTrigger.<Void> create ()));
	}
	...
	public CallbackCompletion<Void> initialize (final CallbackCompletionTrigger<Void> trigger) {
		...
		return (this.connect (AmqpSession.CONNECTOR, new Message (AmqpMessage.ACCESS, null), trigger));
	}
	...
	public CallbackCompletion<Void> publish (final byte[] data, final String exchange, final String routingKey, final String contentType, final String contentEncoding, final boolean mandatory, final boolean immediate, final boolean durable, final String correlation, final String callback) {
		...
		return (this.sendRequest (mssg, token, null));
	}
