...
package eu.mosaic_cloud.platform.implementation.v2.connectors.interop.core;
...
import eu.mosaic_cloud.platform.implementation.v2.connectors.interop.tools.ResponseHandlerMap;
...
public abstract class BaseConnectorProxy
			implements
				eu.mosaic_cloud.platform.implementation.v2.connectors.core.BaseConnectorProxy,
				SessionCallbacks
{
	/**
	...
	protected CallbackCompletion<Void> connect (final SessionSpecification session, final Message initMessage, final CallbackCompletionTrigger<Void> trigger) {
	...
	protected CallbackCompletion<Void> disconnect (final Message finalMessage, final CallbackCompletionTrigger<Void> trigger) {
	...
	protected <TOutcome extends Object> CallbackCompletion<TOutcome> sendRequest (final Message message, final CompletionToken token, final CallbackCompletionTrigger<TOutcome> trigger_) {
		...
		final CallbackCompletionTrigger<TOutcome> trigger;
		if (trigger_ != null)
			trigger = trigger_;
		else
			trigger = CallbackCompletionTrigger.create ();
		this.pendingRequests.register (token.getMessageId (), trigger);
		this.sendMessage (message);
		return (trigger.completion);
	}
