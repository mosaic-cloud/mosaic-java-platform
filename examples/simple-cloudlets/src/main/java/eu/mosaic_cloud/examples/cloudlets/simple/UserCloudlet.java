...
public class UserCloudlet
{
			...
			extends DefaultCloudletCallback<UserCloudletContext>
	{
		...
		public CallbackCompletion<Void> initialize (final UserCloudletContext context, final CloudletCallbackArguments<UserCloudletContext> arguments)
		{
			...
			context.consumer = context.cloudlet.getConnectorFactory (IAmqpQueueConsumerConnectorFactory.class).create (queueConfiguration, AuthenticationToken.class, PojoDataEncoder.create (AuthenticationToken.class), new AmqpConsumerCallback (), context);
			context.publisher = context.cloudlet.getConnectorFactory (IAmqpQueuePublisherConnectorFactory.class).create (queueConfiguration, LoggingData.class, PojoDataEncoder.create (LoggingData.class), new AmqpPublisherCallback (), context);
