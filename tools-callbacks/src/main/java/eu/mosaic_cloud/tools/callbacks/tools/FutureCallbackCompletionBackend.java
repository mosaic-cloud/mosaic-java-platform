...
public final class FutureCallbackCompletionBackend
		implements
			CallbackCompletionBackend
{
	private FutureCallbackCompletionBackend (final ListenableFuture<?> future, final Executor executor)
	{
		super ();
		Preconditions.checkNotNull (future);
		this.future = future;
		this.executor = executor != null ? executor : FutureCallbackCompletionBackend.inlineExecutor;
	}
	
	@Override
	public final boolean awaitCompletion (final CallbackCompletion<?> completion, final long timeout)
	{
		final Object outcome = Threading.awaitOrCatch ((ListenableFuture<Object>) this.future, timeout, FutureCallbackCompletionBackend.timeoutMarker, FutureCallbackCompletionBackend.exceptionMarker);
		if (outcome == FutureCallbackCompletionBackend.timeoutMarker)
			return (false);
		if (outcome == FutureCallbackCompletionBackend.exceptionMarker)
			return (true);
		return (true);
	}
	...
	public final Throwable getCompletionException (final CallbackCompletion<?> completion)
	{
		...
		try {
			outcome = Threading.await ((ListenableFuture<Object>) this.future, 0, FutureCallbackCompletionBackend.timeoutMarker);
	...
	{
		final Object outcome = Threading.awaitOrCatch ((ListenableFuture<Object>) this.future, 0, FutureCallbackCompletionBackend.timeoutMarker, FutureCallbackCompletionBackend.exceptionMarker);
		if (outcome == FutureCallbackCompletionBackend.timeoutMarker)
			throw (new IllegalStateException ());
		if (outcome == FutureCallbackCompletionBackend.exceptionMarker)
			throw (new IllegalStateException ());
	...
	{
		return (CallbackCompletion.createDeferred (new FutureCallbackCompletionBackend (future, null)));
	}
