...
public final class RiakRestDriver extends AbstractKeyValueDriver {
     ...
     */
    private RiakRestDriver(final ThreadingContext threading, final int noThreads,
            final String riakHost, final int riakPort) {
     ...
     * @throws IOException
     */
    public static RiakRestDriver create(final IConfiguration config,
        ...
        RiakRestDriver.logger.trace("Created Riak REST driver for host " + host + ":" + port
                + " [threads=" + noThreads + "]");
    ...
    private static final Logger logger = Transcript.create(RiakRestDriver.class).adaptAs(
            Logger.class);
}
