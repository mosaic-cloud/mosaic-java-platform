...
public class KeyValueStub extends AbstractDriverStub {
     ...
     */
    public KeyValueStub(final IConfiguration config, final KeyValueResponseTransmitter transmitter,
            final AbstractKeyValueDriver driver, final ZeroMqChannel commChannel) {
    ...
    protected void handleKVOperation(final Message message, final Session session,
            final AbstractKeyValueDriver driver,
            final Class<? extends KeyValueResponseTransmitter> transmitterClass)
        ...
        case SET_REQUEST:
            ...
            eu.mosaic_cloud.platform.interop.common.kv.KeyValueMessage messageData = new eu.mosaic_cloud.platform.interop.common.kv.KeyValueMessage(
                    key, data, setRequest.getEnvelope().getContentEncoding(), setRequest
                            .getEnvelope().getContentType());
            KeyValueStub.logger.trace(messagePrefix + kvMessage.toString() + " key: " + key
            ...
            final IResult<Boolean> resultSet = driver.invokeSetOperation(token.getClientId(),
                    messageData, setCallback);
        ...
        case GET_REQUEST:
            ...
            KeyValueStub.logger.trace(messagePrefix + kvMessage.toString() + " key: " + key
                    + " - request id: " + token.getMessageId() + " client id: "
                    + token.getClientId());
            final IResult<eu.mosaic_cloud.platform.interop.common.kv.KeyValueMessage> resultGet = driver
                    .invokeGetOperation(token.getClientId(), key, getCallback);
        ...
        case LIST_REQUEST:
            ...
            KeyValueStub.logger.trace(messagePrefix + kvMessage.toString() + " - request id: "
        ...
        if (unknownMessage) {
            this.handleUnknownMessage(session, driver, kvMessage.toString(), token,
                    transmitterClass);
        }
    }

    protected void handleUnknownMessage(final Session session, final AbstractKeyValueDriver driver,
            final String messageType, final CompletionToken token,
            final Class<? extends KeyValueResponseTransmitter> transmitterClass) {
        KeyValueStub.logger.error("Unexpected message type: " + messageType + " - request id: "
                + token.getMessageId() + " client id: " + token.getClientId());
    ...
    @Override
    protected void startOperation(final Message message, final Session session) throws IOException,
     ...
     */
    public static KeyValueStub create(final IConfiguration config,
            final ThreadingContext threadingContext, final ZeroMqChannel channel) {
    ...
    public static KeyValueStub createDetached(final IConfiguration config,
            final ThreadingContext threadingContext, final ZeroMqChannel channel) {
        KeyValueStub stub;
        try {
            KeyValueStub.logger.trace("KeyValueStub: create new stub."); // $NON-NLS-1$
    ...
    protected static DriverConnectionData readConnectionData(final IConfiguration config) {
        final String resourceHost = ConfigUtils.resolveParameter(config,
                ConfigProperties.getString("KVStoreDriver.0"), String.class, "localhost");// $NON-NLS-1$
                                                                                          // $NON-NLS-2$
        final int resourcePort = ConfigUtils.resolveParameter(config,
                ConfigProperties.getString("KVStoreDriver.1"), Integer.class, 0);// $NON-NLS-1$
        final String driver = ConfigUtils.resolveParameter(config,
                ConfigProperties.getString("KVStoreDriver.6"), String.class, ""); // $NON-NLS-1$
                                                                                  // $NON-NLS-2$
    ...
    private static final Logger logger = Transcript.create(KeyValueStub.class)
            .adaptAs(Logger.class);
    ...
    protected class DriverOperationFinishedHandler implements IOperationCompletionHandler {
        ...
        public DriverOperationFinishedHandler(final CompletionToken complToken,
                final Session session, final Class<? extends AbstractKeyValueDriver> driverClass,
                final Class<? extends KeyValueResponseTransmitter> transmitterClass) {
