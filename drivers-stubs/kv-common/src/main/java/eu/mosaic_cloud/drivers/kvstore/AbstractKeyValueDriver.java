...
public abstract class AbstractKeyValueDriver extends AbstractResourceDriver {
    ...
    public IResult<Boolean> invokeDeleteOperation(final String clientId, final String key,
            final IOperationCompletionHandler<Boolean> complHandler) {
    ...
    public IResult<KeyValueMessage> invokeGetOperation(final String clientId, final String key,
            final IOperationCompletionHandler<KeyValueMessage> complHandler) {
        final IOperationFactory opFactory = this.getOperationFactory(clientId);
        @SuppressWarnings("unchecked")
        final GenericOperation<KeyValueMessage> operation = (GenericOperation<KeyValueMessage>) opFactory
                .getOperation(KeyValueOperations.GET, key);
        return this.startOperation(operation, complHandler);
    }

    public IResult<List<String>> invokeListOperation(final String clientId,
            final IOperationCompletionHandler<List<String>> complHandler) {
        final IOperationFactory opFactory = this.getOperationFactory(clientId);
        @SuppressWarnings("unchecked")
    ...
    public IResult<Boolean> invokeSetOperation(final String clientId, final KeyValueMessage data,
            final IOperationCompletionHandler<Boolean> complHandler) {
        final IOperationFactory opFactory = this.getOperationFactory(clientId);
        @SuppressWarnings("unchecked")
        final GenericOperation<Boolean> operation = (GenericOperation<Boolean>) opFactory
                .getOperation(KeyValueOperations.SET, data);
        return this.startOperation(operation, complHandler);
    }
     ...
     */
    protected <T extends IOperationFactory> T getOperationFactory(final String clientId,
            final Class<T> factClass) {
        T factory = null;
    ...
    @SuppressWarnings({
            "rawtypes", "unchecked" })
    private <T extends Object> IResult<T> startOperation(final GenericOperation<T> operation,
            final IOperationCompletionHandler complHandler) {
