...
public class KeyValueResponseTransmitter extends ResponseTransmitter {
     ...
     */
    public void sendResponse(final Session session, final CompletionToken token,
            final IOperationType operation, final Object result, final boolean isError) {
     ...
     * @return the message
     */
    protected Message buildKeyValueResponse(final KeyValueOperations operation,
        ...
        case GET:
            ...
            @SuppressWarnings("unchecked")
            final Map<String, eu.mosaic_cloud.platform.interop.common.kv.KeyValueMessage> resMap = (Map<String, eu.mosaic_cloud.platform.interop.common.kv.KeyValueMessage>) result;
            final List<KVEntry> getResults = new ArrayList<KVEntry>();
            for (final Map.Entry<String, eu.mosaic_cloud.platform.interop.common.kv.KeyValueMessage> entry : resMap
                    .entrySet()) {
                final KVEntry.Builder kvEntry = KeyValuePayloads.KVEntry.newBuilder();
                kvEntry.setKey(entry.getKey());
                IdlCommon.Envelope.Builder envelope = IdlCommon.Envelope.newBuilder();
    ...
    protected void packAndSend(final Session session, final CompletionToken token,
            final KeyValueOperations operation, final Object result, final boolean isError) {
        Message message;
