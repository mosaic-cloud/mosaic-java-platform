 ...
 * the memcached protocol. This is used by the {@link GenericKvStoreConnector} to
 ...
 * 
 */
public class MemcacheKvStoreConnectorProxy<T extends Object>
		extends BaseKvStoreConnectorProxy<T>
{
	protected MemcacheKvStoreConnectorProxy (final IConfiguration configuration, final Channel channel, final DataEncoder<T> encoder)
	{
	...
	public CallbackCompletion<Boolean> add (final String key, final int exp, final T data)
	{
		...
		requestBuilder.setExpTime (exp);
		{
			byte[] dataBytes;
			try {
				dataBytes = this.encoder.encode (data);
	...
	protected void processResponse (final Message message)
	{
		...
		switch (kvMessage) {
			case GET_REPLY : {
				...
				this.logger.debug ("KvStoreConnectorProxy - Received " + message.specification.toString () + " response [" + token.getMessageId () + "]...");
	 ...
	 * @throws Throwable
	 */
	public static <T extends Object> MemcacheKvStoreConnectorProxy<T> create (final String bucket, final IConfiguration configuration, final String driverIdentity, final Channel channel, final DataEncoder<T> encoder)
	{
		final MemcacheKvStoreConnectorProxy<T> proxy = new MemcacheKvStoreConnectorProxy<T> (configuration, channel, encoder);
		...
		proxy.connect (driverIdentity, MemcachedSession.CONNECTOR, new Message (KeyValueMessage.ACCESS, requestBuilder.build ()));
		return proxy;
	}
}
